...
// --- ROTA DE CARROS (ATUALIZADA PARA AUDIO) ---
app.get('/carros', async (req, res) => res.json(await Carro.find()));

// Configuração para receber Múltiplas Imagens E um Áudio
...
app.post('/carros', cpUpload, async (req, res) => {
    try {
        ...
        await Carro.create({
            ...
            imagens: imagensUrls,
            audioUrl: audioUrl, // Salva o áudio
...
app.put('/carros/:id', cpUpload, async (req, res) => {
    try {
        ...
        if (req.files['imagens'] && req.files['imagens'].length > 0) {
            const novasImagens = req.files['imagens'].map(f => `/uploads/${f.filename}`);
...
app.post('/posts/like/:id', async (req, res) => { try { if(mongoose.isValidObjectId(req.params.id)) { const p = await Post.findById(req.params.id); if(p){ p.likes++; await p.save(); if(p.emailAutor) notificar('like', {nome:req.body.quemDeuLikeNome,email:req.body.quemDeuLikeEmail,avatar:req.body.quemDeuLikeAvatar}, p.emailAutor, 'curtiu.', p.midiaUrl); res.send('Ok'); } } } catch(e) { res.status(500).send('Erro'); } });
app.post('/posts/comentar/:id', async (req, res) => { try { if(mongoose.isValidObjectId(req.params.id)) { const p = await Post.findById(req.params.id); if(p){ p.comentarios.push(req.body); await p.save(); if(p.emailAutor) notificar('comentario', {nome:req.body.autor,email:req.body.emailAutor,avatar:req.body.avatar}, p.emailAutor, `comentou: "${req.body.texto}"`, p.midiaUrl); res.json(p.comentarios); } } } catch(e) { res.status(500).send('Erro'); } });
