...
// --- ROTAS CARROS (CORRIGIDO PARA MÚLTIPLAS E EDITAR) ---
...
// NOVA ROTA DE EDIÇÃO (PUT)
app.put('/carros/:id', upload.array('imagens', 12), async (req, res) => {
    try {
        const carroAntigo = await Carro.findById(req.params.id);
        if(!carroAntigo) return res.status(404).send('Carro não encontrado');
        ...
        // Se enviou novas imagens, substitui as antigas (ou adiciona, aqui vamos substituir para simplificar a edição)
        if (req.files && req.files.length > 0) {
            const novasImagens = req.files.map(f => `/uploads/${f.filename}`);
            updateData.imagens = novasImagens;
            updateData.imagemUrl = novasImagens[0];
        }

        await Carro.findByIdAndUpdate(req.params.id, updateData);
        res.send('Ok');
    } catch(e) { res.status(500).send("Erro"); }
});
...
app.post('/posts/like/:id', async (req, res) => { try { if(mongoose.isValidObjectId(req.params.id)) { const p = await Post.findById(req.params.id); if(p){ p.likes++; await p.save(); if(p.emailAutor) notificar('like', req.body, p.emailAutor, 'curtiu.', p.midiaUrl); res.send('Ok'); } } } catch(e) {} });
app.post('/posts/comentar/:id', async (req, res) => { try { if(mongoose.isValidObjectId(req.params.id)) { const p = await Post.findById(req.params.id); if(p){ p.comentarios.push(req.body); await p.save(); if(p.emailAutor) notificar('comentario', req.body, p.emailAutor, 'comentou.', p.midiaUrl); res.json(p.comentarios); } } } catch(e) {} });
