...
const PostSchema = new mongoose.Schema({
    ...
    comentarios: [{ autor: String, emailAutor: String, avatar: String, texto: String }], // Adicionado emailAutor
...
const CarroSchema = new mongoose.Schema({
    ...
    imagemUrl: String, imagens: [String], descricao: String, specs: Object, mods: [String],
    timestamp: { type: Date, default: Date.now }
});
...
// CORREÇÃO: Função notificar agora é mais robusta
async function notificar(tipo, deObj, paraEmail, texto, img = null) {
    if (deObj.email === paraEmail) return; // Não notificar a si mesmo
    
    // Verifica se já existe uma notificação igual nos últimos 5 minutos para evitar spam
    const recente = await Notificacao.findOne({ 
        tipo, 
        de: deObj.nome, 
        para: paraEmail, 
        texto: texto,
...
app.post('/carros', upload.array('imagens', 12), async (req, res) => {
    try {
        ...
        await Carro.create({
            ...req.body, imagemUrl: imagemPrincipal, imagens: imagensUrls,
            mods: req.body.mods ? req.body.mods.split(',') : [],
            specs: { hp: req.body.potencia, torque: req.body.torque, zero_cem: req.body.zero_cem, top_speed: req.body.top_speed, cor: req.body.cor, ano: req.body.ano, motor: req.body.motor, cambio: req.body.cambio, tracao: req.body.tracao, peso: req.body.peso }
        });
        ganharXP(req.body.emailDono, 100); res.status(201).send('Ok');
    } catch(e) { res.status(500).send("Erro"); }
});
app.put('/carros/:id', upload.array('imagens', 12), async (req, res) => {
    try {
        const updateData = { ...req.body };
        updateData.specs = { hp: req.body.potencia, torque: req.body.torque, zero_cem: req.body.zero_cem, top_speed: req.body.top_speed, cor: req.body.cor, ano: req.body.ano, motor: req.body.motor, cambio: req.body.cambio, tracao: req.body.tracao, peso: req.body.peso };
        updateData.mods = req.body.mods ? req.body.mods.split(',') : [];
        if (req.files && req.files.length > 0) {
            const novasImagens = req.files.map(f => `/uploads/${f.filename}`);
            updateData.imagens = novasImagens; updateData.imagemUrl = novasImagens[0];
        }
        await Carro.findByIdAndUpdate(req.params.id, updateData); res.send('Ok');
    } catch(e) { res.status(500).send("Erro"); }
});
...
// POSTS E LIKES (CORRIGIDO PARA MAPEAMENTO DE NOMES)
...
// LIKE CORRIGIDO: Mapeia quemDeuLikeNome para o formato esperado pela notificação
    ...
    try {
        if (mongoose.isValidObjectId(req.params.id)) {
            ...
            if (p) {
                ...
                await p.save();
                if(p.emailAutor) {
                    // Mapear dados recebidos do frontend
                    const quemDeuLike = {
                        nome: req.body.quemDeuLikeNome,
                        email: req.body.quemDeuLikeEmail,
                        avatar: req.body.quemDeuLikeAvatar
                    };
                    notificar('like', quemDeuLike, p.emailAutor, 'curtiu a tua publicação.', p.midiaUrl);
                }
                res.send('Ok');
            }
        }
...
app.post('/posts/comentar/:id', async (req, res) => {
    try {
        if (mongoose.isValidObjectId(req.params.id)) {
            const p = await Post.findById(req.params.id);
            if (p) {
                p.comentarios.push(req.body);
                await p.save();
                if(p.emailAutor) {
                    // Mapear dados do autor do comentário (req.body.autor = nome)
                    const quemComentou = {
                        nome: req.body.autor,
                        email: req.body.emailAutor || 'anon', // Se o frontend mandar
                        avatar: req.body.avatar
                    };
                    notificar('comentario', quemComentou, p.emailAutor, `comentou: "${req.body.texto}"`, p.midiaUrl);
                }
